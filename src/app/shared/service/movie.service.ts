...
export class MovieService {
  ...
  getMovies(ids: number[], language: string): Promise<Movie[]> {
    const obs = ids.map(id => this.getMovie(id, new DetailConfig(true, true, true, true, true, true, true, true, false, false, language), false));
  ...
  getMovie(id: number, config: DetailConfig, detail: boolean): Promise<Movie> {
    return this.serviceUtils.getPromise(UrlBuilder.detailUrlBuilder(true, id, config.video, config.credit,
      config.reco, config.release, config.keywords, config.similar, config.img, config.titles, false, config.list, config.lang))
      .then(response => {
        ...
        if (detail && (!movie.overview || ((movie.videos === undefined || movie.videos.length === 0) && config.video) || !movie.original_title)) {
          return this.getMovie(id,
            new DetailConfig(false, false, false, false, config.video, false, false, false, false, false, 'en'), false).then(enMovie => {
